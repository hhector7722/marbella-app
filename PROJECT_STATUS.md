...
- [x] **Refinamiento UI Historial (Ajuste Final de Densidad)**: Optimización de espaciados en vista escritorio (grid), consolidando etiqueta, valor y porcentaje en una sola fila para maximizar densidad y subir el contenido inferior.
...
- [x] **Rediseño Tarjeta Producto (Pedidos)**: Evolución a **Plantilla Galería Cuadrada** (estilo `/recipes`). Optimización de la altura total y posicionamiento de imagen/nombre en la zona inferior (justo encima del área petróleo). Ajuste de simetría perfecta en controles: distancias idénticas entre márgenes y botones `+/-` mediante `justify-between` y padding balanceado. Signos flotantes sobre fondo petróleo sin recortes.

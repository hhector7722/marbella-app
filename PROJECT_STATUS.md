...
- [x] **Refinamiento de Tarjetas de Historial (Ajuste Final)**: Ajuste de espaciados y alineación de métricas para maximizar la densidad de información en la vista de escritorio y simplificar la visualización en móviles.
...
- [x] **Rediseño Tarjeta Producto (Pedidos)**: Evolución a **Plantilla Galería Cuadrada** (estilo `/recipes`). Optimización de la altura total y posicionamiento de imagen/nombre en la zona inferior. Ajuste de **simetría perfecta y visibilidad**: centrado de controles mediante `justify-center`, reducción de paddings laterales (`px-1`) y anchos de input para garantizar que el signo `+` sea totalmente visible en móviles y las distancias a los márgenes sean idénticas. Signos flotantes sin recortes.

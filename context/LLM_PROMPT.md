...
**Fuente**: `PROJECT_STATUS.md` — **última actualización:** 2026-06-14 (Actividades pabellón — PDF Gmail + calendario)

Hitos recientes (mismo orden que el changelog superior de `PROJECT_STATUS.md`; máx. 45 entradas):

- **Actividades pabellón: PDF diarios desde Gmail + `/staff/actividades` (2026-06-14)**: Migración [`20260614140000_pavilion_activities.sql`](supabase/migrations/20260614140000_pavilion_activities.sql) — tabla `pavilion_activity_sheets`, bucket Storage `pavilion_activities`. **Ingesta vía Google Apps Script** (mismo patrón que nóminas/albaranes): [`context/script_actividades.txt`](context/script_actividades.txt) → webhook [`/api/webhooks/pavilion-activities`](src/app/api/webhooks/pavilion-activities/route.ts) con `WEBHOOK_SECRET`. Remitente `fmarco@cemmarbella.cat` en bandeja `hhector7722@gmail.com`. Página calendario [`/staff/actividades`](src/app/staff/actividades/page.tsx); acceso en cabecera modal Horarios. Modal PDF zoom [`PavilionActivityPdfModal`](src/components/pavilion/PavilionActivityPdfModal.tsx). Solo Hector: subida manual / pegar PDF.

...
- **Carta embebida: postMessage de navegación al parent (2026-06-02)**: Nuevo componente cliente [`IframeNavBridge.tsx`](src/components/IframeNavBridge.tsx) que, cuando la carta está dentro de un iframe, ejecuta `window.parent.postMessage({ type:'marbella:carta:navigation', pathname })` al cargar, en cada navegación App Router y en `hashchange`. Montado en [`src/app/carta/layout.tsx`](src/app/carta/layout.tsx). `targetOrigin` recomendado vía `NEXT_PUBLIC_MARBELLA_WEB_ORIGIN` (fallback inferido por `document.referrer` o `*`).

...
- **La vista de plantilla (todos los trabajadores) comparte el cromo y no el pie.** Primer nombre y fichajes por día. No se unifica el contenido de celda con el de una persona. Cada registro es una zona hundida (`color.superficie.inactiva`) a ancho de columna, sin contorno, sin relleno vertical ni radio de `Surface`. Nombre a la izquierda; horas, inicial F/E/B/P o cruz de no registrado justificadas a la derecha, con `espacio.1` de margen al canto de la columna.

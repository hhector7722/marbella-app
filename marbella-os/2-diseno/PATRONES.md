...
- **La vista de plantilla (todos los trabajadores) comparte el cromo y no el pie.** Primer nombre y fichajes por día. No se unifica el contenido de celda con el de una persona. Cada registro es una zona hundida (`color.superficie.hundida`) que abraza nombre y marca: sin relleno, sin radio de `Surface`. Nombre a la izquierda y marca a la derecha en esa misma card: horas, inicial F/E/B/P o cruz de no registrado.

...
**Reglas**:
- Respeta el área segura del dispositivo y usa el alto visible real, nunca el teórico. El tope de alto del panel es el interior del overlay (`min(100%, estructura.alto-modal)`): viewport visible menos padding y área segura. El contenido que no cabe se desplaza en el Body; el panel no se recorta.
- Atenúa y desactiva las barras fijas de la aplicación mientras está abierto.
- Cabecera fija (`estructura.cabecera-modal` = 36px) y pie no se desplazan; el pie no se encoge. El contenido de cabecera (título, iconos, acciones) se adapta tipográficamente; no se trunca el título por la altura. Botones de cabecera (chrome) independientes de Button, cuadrados al alto de la cabecera. La cabecera de trabajo es superficie, no franja de marca. Los overlays de **elección y menú de acceso**, el **modal de día** (horario, asistencia) y las fichas de consulta (Datos personales) usan `scheme="dark"` (envolvente bajo, tinta invertida). Los valores viven en papel (`color.superficie`). Formularios de recuento (caja, albaranes) siguen `scheme="work"` (blanco).
...
- **Escritorio: el espacio extra sirve para leer, no para maximizar el panel.** `work` sigue siendo el panel ancho de tablas densas. `day` se dimensiona al contenido de la fecha (nombres + escala horaria), `max-w-4xl`. La altura crece con filas y franjas legibles (`tipo.minimo`); no se estira al viewport por defecto. El scroll interno es el último recurso cuando el día no cabe en `--modal-max-height`.
...
Los dos flotantes miden lo mismo y viven en un **dock vertical**. No son cromo de Modal. La calculadora va **encima** del desglose cuando coinciden. El dock es **arrastrable por toda la altura** de la pantalla y la posición elegida se recuerda por dispositivo. Por defecto se sienta sobre la **TabBar**, con el aire de `espacio.2`; si la cápsula está oculta o no hay TabBar, baja al área segura. **Nunca la tapa**: el arrastre se topa en ese canto inferior. Dentro de ese límite, la posición vertical la fija el usuario; el sistema no la fuerza. **No restan ancho a ningún modal** ni entran en su flujo: si el acceso vive dentro de un Modal, el dock se superpone al canto derecho de ese panel, sigue visible al desplazar el cuerpo y cada botón lleva un contorno blanco (`color.superficie`) que no cambia su caja de 48 px. `--quick-fab-lift` solo coloca el canto inferior del dock. Con el panel abierto, el dock se retira: no se ve y no tapa teclas ni rejilla. Un chevron sutil (V) en el canto superior, de 48 px de toque, minimiza el panel y los devuelve.

La calculadora reproduce el teclado de la imagen de producto: teclas pastilla (anchas, no círculos) de 48 px de alto, números, AC, retroceso, %, ⁺⁄₋, coma y operadores naranja. El reloj oculta las teclas y muestra el historial de cálculos; copiar pone el valor actual en el portapapeles. La estética iOS de este teclado es excepción explícita de producto: no usa tokens de petróleo. El desglose de borrador reutiliza `DenominationCountGrid` (el mismo del arqueo), compactado a cinco columnas y a la misma altura que la calculadora. El recuento de caja no usa esa densidad.
...
| `tall` | 1×2 | widget de una columna y dos filas |

El widget no elige un ancho libre: entra en la misma retícula. Las tres homes usan el mismo lienzo, el mismo hueco y el mismo aire. En smartphone no hay 7 u 8 columnas. En escritorio (lg+) sí: una plantilla única de 8×5; el contenido ocupa regiones de esa retícula, no se redistribuye con flex-wrap ni con columnas distintas por página. `label` en el slot pinta el nombre bajo el hueco. Sin `label`, el widget mide lo mismo que icono + nombre. Entre filas, después de los nombres, hay el mismo aire que entre columnas. El hueco del widget **es material de sistema** (cristal que toma el wallpaper; claro u oscuro según el fondo; el contenido vive en el vidrio). En oscuro la fuente del contenido es blanca: no zinc, no gris, no marca. Un solo canto: hairline del cristal, no el hilo de 2 px del icono. El canto del icono (`data-element="rim"`) sigue en `DashboardShortcut`. El pie de la semana Staff en el mosaico baja el peso (600, no 900) para que PENDIENTES se lea. Admin monta `OpsHomeScreen` (Ventas, Último cierre, Horario 3x2 con C inicial y Plantilla a la derecha, M obra/Stock/Recetas/Albaranes en la fila 5, y Otros/Asistencia/Cajas cambio/Ingredientes en la fila 6). En escritorio Admin usa la plantilla 8×5: Ventas 3×2, Último cierre 3×2, Horario 6×3 y diez accesos en las dos columnas derechas; C Inicial queda en la misma fila que Plantilla y Cajas Cambio en la misma fila que Ingredientes porque ocupan esos slots, no por coordenadas sueltas. Master monta Ventas (4×1, fila 1), el widget «Último cierre» (4×1, fila 2: el cierre más reciente, con la misma magnitud que la card de `/dashboard/history`), la asistencia del día en curso y Horarios sobre la misma `HomeScreen` (asistencia 1×1, del tamaño de icono+nombre, en la fila 3, columna 4; horarios 3×2 en las filas 3–4, columnas 1–3), sus atajos (Caja Inicial como dos en uno — importe verde que abre movimientos y Acción naranja que abre Entrada/Salida/Compra/Arqueo —, Cajas Cambio también como dos en uno: un mini icono por caja, Cambio 1 arriba y Cambio 2 abajo, cada uno con su importe sobre verde y abriendo el arqueo de su caja) y el acceso «Otros», que abre un modal con los accesos retirados del mosaico: proveedores, web, carta, consumo, horarios, asistencia, cierre, propinas, uso app, rentabilidad y albaranes. En escritorio Master usa la misma plantilla 8×5: Ventas 3×2, Último cierre 3×2, Horario 6×3, Asistencia 2×1 en las dos columnas de accesos de la fila 1; Albaranes no ocupa un slot del mosaico desktop (sigue en Otros y en el smartphone). La asistencia del Master es el día en curso de todos los trabajadores: franja roja compacta con la fecha (formato «Sábado 5 de sept», en una sola fila) y bajo ella el nombre, la entrada y la salida de cada uno; si el día en curso no tiene fichajes, arranca en el último día con registros. La franja y cualquier registro abren el resumen de fichajes (DaySummaryModal, el mismo que `/staff/history` al pulsar un día); desde el resumen se llega al detalle del trabajador. La navegación entre días se hace por las esquinas inferiores del widget (izquierda = día anterior, derecha = día siguiente, sin pasar de hoy), superpuestas a la lista sin ocupar altura, y no lleva ningún indicador. La tipografía es compacta por excepción (7 px datos, 6 px fecha; ver TOKENS) y las horas van en formato H, con minutos solo cuando son :30, pegadas al guion sin espacios. Si hay hora de entrada pero no de salida, se muestra solo la entrada, sin guion. El widget no se despliega automáticamente al desbordar: si caben en el alto compacto ocupa una celda 1×1 con el nombre «Asistencia» bajo el hueco; si desbordan, muestra un indicador de despliegue muy sutil en la zona inferior y, al pulsarlo, crece a 1×2 (filas 3–4, columna 4) y el nombre desaparece. En escritorio no se despliega: el widget llena su zona 2×1. Sus filas de trabajador son compactas, por debajo del mínimo táctil, por densidad del mosaico (decisión de la propiedad); el conjunto de registros se centra verticalmente en la zona blanca cuando no la llena. La semana de plantilla de `/staff/history` y el resumen semanal del mosaico Staff no cambian. Staff monta Semana, Entrada y Horarios sobre la misma `HomeScreen`. En escritorio Staff usa la plantilla 8×5: resumen semanal 5×2, Horario 5×3 y quince accesos (tres columnas); el slot libre es Perfil, el mismo destino que el Perfil del TabBar (`/profile`), con el mismo recurso visual que Plantilla. En escritorio, el hueco exterior de Ventas, Último cierre, Horario y el resumen semanal Staff ya lo cierra la plantilla 8×5: el contenido interno se reparte en ese hueco (cabecera, información secundaria, contenido principal, complemento) y no se compacta arriba dejando el resto vacío. En Horario, las semanas cerradas no crecen a costa de las cards de sábado y domingo: la semana abierta reserva alto para título, evento y horas. En esas cards, la fila de horario del evento, participantes y categoría reparte el ancho en tres columnas iguales, con cada valor centrado. El horario del evento va sin cero inicial: `8h` si los minutos no son 30, `8:30` si lo son. La misma pieza usa la misma composición interna en Admin y en Master.
...
- T1 (mosaico Staff/Admin/Master) no es PageScreen: es `HomeScreen` (4×6 en smartphone, 8×5 en escritorio lg+) con `DashboardShortcut` y widgets en huecos.
...
|---|---|---|---|
| T1 | Dashboard mosaico | HomeScreen + DashboardShortcut + widgets en huecos. **No** PageScreen | 4×6 smartphone; 8×5 escritorio (lg+); pista = squircle; widget 2×2 / 4×2 / 4×4 / 4×1 / 2×1 / 3×2 |

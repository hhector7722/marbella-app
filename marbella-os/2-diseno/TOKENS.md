...
| `color.superficie.hundida` | `#FAFAFA` | Fondo de zonas agrupadas dentro de una superficie | adoptado (variable `--color-superficie-hundida`) |
| `color.superficie.inactiva` | `#F4F4F5` | Elementos deshabilitados, cabeceras de tabla, filas de plantilla en Asistencia | adoptado (piloto Button vía `--color-superficie-inactiva`; fila `plantilla-log-row`) |
...
| `estructura.alto-modal` | `100dvh − safe − 2rem` (padding del overlay, 1rem por lado). El panel usa `min(100%, token)` respecto al interior del overlay. Si hay iconos de calculadora/desglose, el overlay suma `--quick-fab-dock` y `--quick-fab-lift` (el lift es el área segura; con TabBar visible, también la cápsula). Si el panel de herramienta está abierto, también `--quick-tool-inset`. Otros shells pueden conservar 94% documentado históricamente | Alto máximo de un modal de tarea: el área visible, no un recorte del dvh teórico | adoptado (`--modal-max-height`) |
...
- **El widget de asistencia del Master y las etiquetas de día especial del calendario usan tipografía por debajo de `tipo.minimo` por excepción.** En el widget Master: datos (nombre y horas) a 7 px y fecha a 6 px. En el calendario de asistencia (historial y resumen semanal del mosaico Staff): etiquetas Festivo / Enfermo / Baja / Personal a 7–10 px según longitud de la celda (`SpecialDayLabel`). Densidad de mosaico/celda, decidida por la propiedad del producto. Documentado en [SISTEMA-DE-COMPONENTES](SISTEMA-DE-COMPONENTES.md).

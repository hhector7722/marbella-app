...
**Criterio de autoridad cuando hay duplicación:** manda el motor en TypeScript, porque es el que tiene pruebas. Las funciones equivalentes en base de datos son atajos de consulta y deben dar el mismo resultado. Si discrepan, la función está mal. Excepción ya cerrada para el coste completo de una elaboración: la autoridad es `get_recipe_cost_v2`. La expansión física hasta materia prima es `get_recipe_stock_requirements_v2`; no hay un segundo motor en TypeScript. `recipe_stock_requirements_v2_rows` lee ese JSON y no recorre la receta por su cuenta. `process_ticket_stock_deduction` escribe desde esas filas: si `ok` es false aborta el ticket entero y no deja corrida, si la expansión válida está vacía no bloquea. Un ticket sin líneas persistidas no se cierra. Si hay líneas y no sale materia prima, la corrida en `private.ticket_stock_deduction_runs` cierra el ticket con recuento cero. Un SALE antiguo sin corrida no se reabre ni se rellena. `processRecipeWaste` escribe esas mismas materias primas con `record_waste_movements`; si la expansión válida está vacía, la merma manual no se da por hecha. `process_staff_consumption`, en ración completa, escribe esas materias primas como `WASTE` de consumo personal. Si esa receta no expande bien, el error queda en `staff_consumption_register_errors` y el resto del carrito sigue. La media ración directa conserva `quantity_half` y convierte a `ingredients.base_unit`. La media ración con subrecetas no se escribe. Una elaboración interna no es un producto de consumo personal. La hoja de ingrediente de ese motor tiene que coincidir con `getRecipeIngredientLineCostV2`. `get_recipe_cost` queda como lector legacy y no es esa autoridad.

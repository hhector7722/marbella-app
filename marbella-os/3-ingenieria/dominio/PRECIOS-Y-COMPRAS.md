...
Cuando la línea apunta a otra receta, el coste y el stock no se detienen ahí: se expanden hasta el ingrediente y el resultado se reparte según el rendimiento. Ese contrato está en [ADR-0018](../../4-decisiones/ADR-0018-elaboraciones-intermedias.md). El coste completo lo produce `get_recipe_cost_v2`. En su resultado, `components[].cost_eur` es la aportación a la receta pedida, ya escalada por cada rendimiento; `local_cost_eur` es el coste del tramo antes de ese reparto. La ficha de una elaboración interna lee `get_recipe_cost_v2`. La ficha vendible y el resto de lectores siguen en `get_recipe_cost`, que no recorre subrecetas. El stock tampoco usa todavía esta expansión ([D32](../../5-estado/DEUDA.md)). Este apartado sigue gobernando la conversión de la línea de ingrediente.

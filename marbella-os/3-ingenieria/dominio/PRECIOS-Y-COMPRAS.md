...
Cuando la línea apunta a otra receta, el coste y el stock no se detienen ahí: se expanden hasta el ingrediente y el resultado se reparte según el rendimiento. Ese contrato está en [ADR-0018](../../4-decisiones/ADR-0018-elaboraciones-intermedias.md). El coste completo lo produce `get_recipe_cost_v2`. En su resultado, `components[].cost_eur` es la aportación a la receta pedida, ya escalada por cada rendimiento; `local_cost_eur` es el coste del tramo antes de ese reparto. La ficha edita ingredientes y elaboraciones como componentes distintos. `recipe_subrecipes` no es `recipe_combos`. La elaboración interna, y la vendible con subrecetas en ración completa, leen `get_recipe_cost_v2`. La vendible sin subrecetas sigue en `get_recipe_cost`. Si hay subrecetas y la vista es media ración, el coste queda indeterminado: no hay media cantidad en `recipe_subrecipes`. El stock, el TPV y las ventas tampoco usan todavía esta expansión ([D32](../../5-estado/DEUDA.md)). Este apartado sigue gobernando la conversión de la línea de ingrediente.

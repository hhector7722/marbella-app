...
- **Horario: se estabiliza el renderizado del widget en su primera carga.** En las tarjetas de expansión de fin de semana (`WeekExpansion`) del widget en modo Master, se eliminó la envoltura en un `div` neutro y se aplicó la clase `col-span-7` directamente al componente grid. Esto dota al elemento de dimensiones de altura mínimas y máximas explícitas instantáneas ante el motor CSS de renderizado del navegador, erradicando el bug de solapamiento y pisado de contenido en la primera carga.

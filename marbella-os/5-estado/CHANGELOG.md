...
- **Horario: se incrementa el margen inferior del widget de calendario.** Se modifica la propiedad `--staff-calendar-inset-bottom` en `globals.css` a `var(--espacio-3)` (12px) tanto en la vista Staff como en la de Master. Esto eleva ligeramente los bloques del calendario dejando mayor separación con el borde inferior del widget, respetando siempre la fila superior del título para evitar que se pise.

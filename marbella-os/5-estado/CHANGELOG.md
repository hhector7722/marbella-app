...
- **Horario: se estabiliza el renderizado del widget en su primera carga.** Se unifica la estructura de cada semana bajo el contenedor `week-block` de flexbox para ambos modos (Master y Staff). En modo Master, la fila de días es un grid de 8 columnas donde se aplica la clase `shrink-0` en el contenedor de días y Ext, y la expansión de fin de semana (`WeekExpansion`) se renderiza como hijo directo del contenedor flex, dándole una anchura exacta del `87.5%` (`w-[87.5%] max-w-[87.5%]`) para alinearse con los días y evitar invadir o pisar la columna Ext. Esto soluciona de forma definitiva el colapso o solapamiento de tarjetas y desalineación vertical en la primera carga.

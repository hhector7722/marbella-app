...
- **La exención de deuda de agosto se prorratea por días reales.** En semanas que cruzan julio/agosto o agosto/septiembre, solo los días fuera de agosto pueden generar deuda contractual; ordinarias y extras siguen usando la jornada efectiva completa. [HORAS](../3-ingenieria/dominio/HORAS.md).
...
- **Calculadora y desglose de borrador se abren por botones distintos.** Ya no hay un solo FAB con pestañas de acceso. Recuentos de caja, cierre, cambio (excepto el selector de cajas), tesorería, historial y propinas llevan los dos iconos (`/icons/calculadora.png`, `/icons/desglose.png`). Inventario, horas extras y el resumen de pedido solo llevan calculadora. [SISTEMA-DE-COMPONENTES](../2-diseno/SISTEMA-DE-COMPONENTES.md).
...
- **Horario y Dashboard Staff: carga desacoplada y eliminación del parpadeo de pantalla.** Se independiza la consulta de turnos de la de actividades en `StaffWeekScheduleWidget`, liberando el estado de carga en cuanto llegan los turnos y restringiendo la lectura de actividades exclusivamente al fin de semana de la semana activa. Se elimina el parpadeo de pantalla completa en `/staff/dashboard` quitando el Suspense fallback global de página, fijando el esquema de cristal oscuro por defecto en `HomeScreen` y propagando la identidad del usuario desde el servidor para evitar saltos durante la hidratación.
- **Horario: explicación contextual sobre «Añadir nota» en fin de semana.** En el modal de detalle de sábado y domingo, se muestra una card informativa compacta de fondo blanco y contorno verde que apunta con una flecha al botón «Añadir nota», explicando que se pueden indicar circunstancias puntuales para intentar adaptar el turno si es posible. Se muestra una única vez de forma global compartida entre ambos días y se registra de forma persistente.
...
- **Asistencia: el resumen semanal carga su estructura de forma instantánea.** En el widget de resumen de asistencia semanal (`StaffAttendanceSummaryWidget`), se implementó un esqueleto de datos locales que genera y renderiza de forma instantánea la tabla de días, la cabecera roja y los nombres de los conceptos resumen del pie (como Horas, Bolsa, Extras y Cobro). Muestra un sutil mini-spinner en la esquina superior derecha del widget mientras se descargan y cargan los datos reales.
- **Horario: el calendario del widget se muestra de forma instantánea.** En el widget de horario, se eliminó el spinner a pantalla completa que bloqueaba el renderizado de la cuadrícula durante la carga inicial. Ahora, la estructura base del calendario se dibuja de forma instantánea al montar el componente basándose en los días locales del mes actual y mostrando un sutil mini-spinner en la esquina superior derecha del widget mientras se cargan los turnos y actividades en segundo plano.
...
- **Horario: se estabiliza el renderizado del widget en su primera carga.** Se unifica la estructura de cada semana bajo el contenedor `week-block` de flexbox para ambos modos (Master y Staff). En modo Master, la fila de días es un grid de 8 columnas plano (`grid-cols-8`) con el atributo `data-master-row="true"`. Su fondo de card de semana seleccionada se dibuja mediante un pseudo-elemento en CSS (`::before` con `right: 12.5%`), lo que previene el uso de sub-grids anidados. Esto garantiza una alineación de columnas perfecta al píxel desde el primer frame, erradicando por completo el descuadre visual y el solapamiento en la primera carga.
- **Horario: el horario del evento no se abrevia en modo Master.** En las tarjetas de expansión de fin de semana (sábado y domingo) de `/dashboard` y `/master/dashboard` (`masterMode`), la columna que muestra el horario del evento no se trunca ni se abrevia. Se utiliza un ancho basado en su tamaño real de contenido (`max-content`) y se desactivó el truncamiento de texto (`whitespace-nowrap`), garantizando que las horas sean legibles por completo.
- **Horario: se iguala el diseño de /dashboard con /master/dashboard.** Se unifica la apariencia del widget de horario (`master-horarios`) aplicando el mismo estilo, márgenes, variables CSS de espaciado y estructura `flex-col` al layout de administración (`ops-admin` de `/dashboard`), garantizando que se visualice de manera idéntica al correcto de `/master/dashboard`.
- **Horario: se oculta la fila de turno en fin de semana en modo Master.** En las tarjetas de expansión de sábado y domingo del widget de horario, se oculta la fila correspondiente a «Turno» únicamente cuando se visualiza desde el Master Dashboard (`masterMode`). Esto permite subir el bloque de «Evento» y detalles para un diseño más limpio, manteniendo el widget de Staff Dashboard sin cambios.
- **Horario: el widget muestra una única actividad priorizando pistas y duración.** El calendario del widget de horario (tanto Staff como Master) filtra las actividades diarias para mostrar únicamente una por evento de fin de semana, seleccionando con prioridad la de mayor duración que ocupe las pistas principales (P1, P2, P3 y/o P4) y resolviendo empates por orden de pista (P1 > P2 > P3 > P4). Si no hay actividades en pista, selecciona igualmente la de mayor duración.
- **Perfil: condiciones laborales y contrato se unifican en una sola card colapsable en dos columnas.** El botón «Condiciones laborales» que navegaba a otra página y la card separada «Mi contrato» se unifican para manager en una sola card en el perfil. Muestra de forma integrada las fechas de contrato (en un grid de dos columnas) y un histórico de periodos colapsados por defecto en un acordeón interactivo. Al expandir un periodo, enseña sus condiciones compactadas también en dos columnas (horas, régimen, bolsa, tarifa). Cuenta con un icono de lápiz arriba a la derecha para activar la edición inline en pantalla, ocultándose durante la edición. El resto de usuarios (staff) no ven la sección. [SISTEMA-DE-COMPONENTES](../2-diseno/SISTEMA-DE-COMPONENTES.md).
...
- **Master: la columna «Ext» del horario queda dentro del calendario sin solaparse.** El widget de horario mantiene las 8 columnas alineadas (7 días + Ext); la card de la semana seleccionada cubre solo hasta DOM y la expansión de sáb/dom se limita a las 7 primeras columnas, de modo que la columna de horas extra no se toca ni se pisa con la de domingo ni con la de la semana en curso. Los valores «Ext» se muestran en cards con fondo, vacías cuando no hay importe.
...
- **Horario: la nota del día ya se guarda en modo «ver como».** El master que simula un empleado abría el modal de día desde el widget de horario, pero al guardar una nota el INSERT fallaba: la política de filas exigía `user_id = auth.uid()` y la nota se escribía con el empleado simulado. Las políticas de escritura de `schedule_day_notes` se extienden igual que la de lectura: manager/supervisor/admin pueden escribir la nota de cualquier usuario y día; el staff sigue escribiendo solo la suya.
- **Perfil: al ver otro empleado, Cambiar contraseña y Cerrar sesión se apagan en lugar de ocultarse.** Un manager que visita el perfil de otro usuario ya no pierde esas dos opciones de la rejilla: aparecen con tonalidad apagada (opacidad reducida y cursor inactivo) para indicar que no están activas, y al pulsarlas no ocurre nada. El resto de la cuenta (avatar editable, título «Mi cuenta») sigue ocultándose cuando se ve otro perfil.
- **Perfil: la ficha de «Datos personales» pasa al azul del envolvente.** El modal adopta el `scheme="dark"` de superficie azul marino, desaparece la cabecera petróleo de la ficha y su subtítulo «Ficha de datos personales». Los valores bajan de tamaño y pierden la negrita, los títulos de sección dejan de escribirse en mayúsculas y un campo sin valor se muestra vacío en lugar de «—». [TOKENS](../2-diseno/TOKENS.md), [SISTEMA-DE-COMPONENTES](../2-diseno/SISTEMA-DE-COMPONENTES.md).
...
- **Master: la asistencia del día recupera dos filas (1×2).** El hueco de una sola fila dejaba ver solo los dos primeros registros del día y el resto quedaba oculto sin indicación de scroll; con dos filas se ven todos los trabajadores que ese día tienen fichajes.

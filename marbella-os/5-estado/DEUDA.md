...
**Prioridad: media.** [ADR-0018](../4-decisiones/ADR-0018-elaboraciones-intermedias.md) cierra el modelo de subrecetas de cocina. Hecho: la estructura, `get_recipe_cost_v2`, el rechazo de ciclos al escribir, el motor `get_recipe_stock_requirements_v2` y su lectura en filas `recipe_stock_requirements_v2_rows`, la venta TPV que descuenta esa expansión de forma atómica, la ficha que distingue vendible de elaboración interna, la composición de ingredientes y elaboraciones en `recipe_subrecipes`, y la proyección comercial de carta, que solo muestra recetas con `is_sellable`. Un mapeo TPV hacia una interna no se borra: la carta no lo cuenta. El coste recursivo de la receta completa usa v2 cuando hay subrecetas. La media ración con subrecetas queda indeterminada. Pendiente: migrar Recetas TPV, consumo personal, merma, ranking e importaciones, y retirar `get_recipe_cost` en las vendibles que no tienen subrecetas. Un ticket completado, aunque no haya movido stock, no se reprocesa. Los que ya tenían SALE y no tienen corrida tampoco. `recipe_combos` no paga esta deuda.

**Coste**: una elaboración ya puede entrar en otra receta, costearse en la ficha y descontarse en una venta nueva. Consumo personal y merma siguen sin expandirla. Quien implemente los bloques siguientes tiene que seguir el ADR y no reinterpretarlo.

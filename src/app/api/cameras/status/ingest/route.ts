...
export async function POST(request: NextRequest) {
  ...
  const payload = body as {
    ...
    externalOnline?: unknown;
    checkedAt?: unknown;
  };

  if (
    !Number.isInteger(payload.externalConnections) ||
    (payload.externalConnections as number) < 0 ||
    typeof payload.externalOnline !== 'boolean' ||
  ...
  const { error } = await supabase.from('camera_external_viewer_status').upsert(
    {
      ...
      external_online: payload.externalOnline,

            ...
            <div className="flex-1 overflow-y-auto bg-gray-50 custom-scrollbar">
                <div className="p-4 grid grid-cols-1 gap-4">
                    {DENOMINATIONS.map(denom => {
                        const isBill = denom >= 5;
                        return (
                            <div key={denom} className="grid grid-cols-[1fr_auto_1fr] items-center gap-6 p-2 transition-all">
                                {/* ENTRADA */}
                                <div className="flex flex-col items-center">
                                    <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-2">ENTRA</span>
                                    <input
                                        type="number"
                                        min="0"
                                        ...
                                        placeholder="0"
                                        className="w-full bg-emerald-50/50 border-2 border-transparent focus:border-emerald-200 rounded-2xl p-3 text-center font-black text-emerald-600 outline-none text-sm transition-all shadow-sm"
                                    />
                                </div>
                                ...
                                <div className="flex flex-col items-center justify-center gap-2 min-w-[80px]">
                                    ...
                                    <span className="font-black text-gray-400 text-[10px] tracking-widest uppercase">
                                        {denom >= 1 ? `${denom}€` : `${(denom * 100).toFixed(0)}c`}
                                    </span>
                                </div>
                                ...
                                {/* SALIDA */}
                                <div className="flex flex-col items-center">
                                    <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-2">SALE</span>
                                    <input
                                        type="number"
                                        min="0"
        ...
        <div className="flex flex-col h-full bg-white">
            <div className="bg-[#36606F] px-8 py-4 flex justify-between items-center text-white shrink-0">
                <div className="flex flex-col">
                    ...
                    <h3 className="text-lg font-black uppercase tracking-wider leading-none mt-1">{boxName}</h3>
                </div>
                <button onClick={onBack} className="w-10 h-10 flex items-center justify-center bg-white/10 rounded-xl hover:bg-white/20 transition-all text-white active:scale-90">
            ...
            <div className="p-6 bg-gray-50 border-b flex justify-between items-center shrink-0">
                ...
                <span className="text-3xl font-black text-[#36606F]">{total.toFixed(2)}€</span>
            </div>
                    ...
                    {inventory.sort((a, b) => b.denomination - a.denomination).map((item, idx) => (
                        <div key={idx} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
                            ...
                            <div className="text-right">
                                <span className="text-xl font-black text-[#36606F] leading-none">x{item.quantity}</span>
                                <p className="text-[8px] font-bold text-gray-300 uppercase tracking-tighter">{(item.denomination * item.quantity).toFixed(2)}€</p>
                            </div>
                        </div>
                    ))}
                    {inventory.length === 0 && (

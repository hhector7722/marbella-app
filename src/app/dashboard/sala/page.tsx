    ...
    async function fetchHourly() {
      try {
        ...
        const { data: ticketsData } = await supabase
          ...
          .gte('fecha', chartDate)
          .lte('fecha', chartDate)
          .limit(5000); // MODIFICACIÓN: Límite para gráfica
        const hourly = Array.from({ length: 24 }, (_, h) => ({ hora: h, total: 0 }));
  ...
  async function fetchVentas() {
    ...
    try {
      ...
      const ticketsPromise = supabase
        ...
        .limit(5000); // MODIFICACIÓN: Límite para listado
      ...
      if (ticketsRes.error) {
        if (ticketsRes.error.code === '42P01') {
          console.warn("Tabla tickets_marbella no detectada o permisos erróneos.");
        } else {
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden">
          ...
          {(() => {
            ...
            const handleChartTap = (clientX: number) => {
              ...
              const hour = BUSINESS_HOURS.start + Math.round(ratio * (numPoints - 1));
              const finalH = Math.min(maxSelectableHour, Math.max(BUSINESS_HOURS.start, hour));
              setSelectedChartHour(finalH);
            };
            const totalHastaHora = selectedChartHour === null ? 0 : Array.from(
              { length: selectedChartHour - BUSINESS_HOURS.start + 1 },
              (_, i) => salesChartData[BUSINESS_HOURS.start + i]?.total ?? 0
            ...
            return (
              <div className="w-full pb-1 pt-2 px-4 border-b border-zinc-50 shrink-0 print:hidden">
                ...
                {selectedChartHour !== null && typeof document !== 'undefined' && (() => {
                  ...
                  const yView = 22 - ((salesChartData[selectedChartHour]?.total ?? 0) / scaleMax) * 18;

...
import { valuesEqual } from '@/lib/design-system/visual-studio/ux-scenes';
import {
    propertiesForStyle,
    STYLE_TYPES,
    styleTypeById,
    ...
    type StyleType,
    type StudioScreen,
} from '@/lib/design-system/visual-studio/ux-styles';
import {
    formatStudioStamp,
    humanConsumerName,
    humanDebtLead,
...
import { StudioStylePreview } from './studio-style-preview';
...
export function DesignSystemStudio({
    ...
    const [styleId, setStyleId] = useState(STYLE_TYPES[0]!.id);
    const [previewId, setPreviewId] = useState(STYLE_TYPES[0]!.id);
    ...
    const style = styleTypeById(styleId);
    const previewStyleType = screen === 'home' ? styleTypeById(previewId) : style;
    const element = elements.find((item) => item.id === style.elementId) ?? elements[0]!;
    const actual = useMemo(() => actualValues(element), [element]);
    const proposalValues = { ...actual, ...(drafts[element.id] ?? {}) };
    const visibleProperties = propertiesForStyle(element, style);
    ...
    const readOnlyKind =
        element.status === 'DEPRECADO' || element.status === 'ESPECIALIZADO' || inherited || Boolean(style.decided);
    const canEdit =
        !readOnlyKind &&
    ...
    const changes = describeChanges(element, proposalValues).filter((change) =>
        visibleProperties.some((property) => property.label === change.property)
    );
    const canSaveProposal = canEdit && visibleProperties.length > 0;
    ...
    useEffect(() => {
        ...
        if (!pieza) return;
        openStyle(styleTypeForParam(pieza).id);
    ...
    function openStyle(id: string) {
        const next = styleTypeById(id);
        const item = elements.find((entry) => entry.id === next.elementId);
        if (!item) return;
        setStyleId(next.id);
    ...
    const showPreview = screen === 'home' || screen === 'edit';
            ...
            <div className="flex min-h-0 flex-1 flex-col md:flex-row">
                {showPreview ? (
                    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                        <div className="shrink-0 space-y-ds-2 px-ds-4 pt-ds-3">
                            {screen === 'home' ? (
                                <p className="m-0 text-[14px] text-ds-texto">
                                    Un tipo, toda la app. Cambia un botón de guardar y cambian todos.
                                </p>
                            ) : null}
                            {dirtyUnsaved && screen === 'edit' ? (
                            ...
                            {hasSavedEssay && !dirtyUnsaved && screen === 'edit' ? (
                        ...
                        {screen === 'home' ? (
                            <div className="shrink-0 border-t border-ds-borde px-ds-4 py-ds-3 pb-[max(env(safe-area-inset-bottom),0.75rem)] md:hidden">
                                <Button
                                    variant="secondary"
                ...
                {screen === 'home' ? (
                    <aside className="flex min-h-0 flex-[1.1] flex-col overflow-hidden border-t border-ds-borde bg-ds-superficie md:max-w-[24rem] md:border-l md:border-t-0">
                        <StyleList
                            styleId={previewId}
                            onOpen={openStyle}
                            onHover={setPreviewId}
                            essaysCount={essays.length}
                            onEssays={() => setScreen('essays')}
                        />
                    </aside>
                ) : null}
                        ...
                        <EditBody
                            ...
                            footer={
                                <SheetFooter
                                    ...
                                    gateOk={gate.ok}
                                    hasProperties={visibleProperties.length > 0}
                ...
                {screen === 'essays' ? (
                    <EssaysScreen
                        ...
                        onOpen={(id) => openStyle(styleTypeForParam(id).id)}
                    />
                ) : null}
...
function StyleList({
    ...
    return (
        <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 space-y-ds-2 overflow-auto p-ds-4">
                <p className="m-0 text-[12px] text-ds-texto-tenue">
                    Elige un tipo. El cambio pinta todos los de ese tipo.
                </p>
                <ul className="m-0 grid grid-cols-1 gap-ds-2 p-0">
                    {STYLE_TYPES.map((item) => (
                        <li key={item.id}>
                            <button
                                type="button"
                                ...
                                className={`flex w-full min-h-ds-tactil shrink-0 flex-col items-start justify-center gap-ds-1 border px-ds-4 py-ds-3 text-left ${
                                    item.id === styleId
            ...
            <div className="hidden shrink-0 border-t border-ds-borde p-ds-4 md:block">
                <Button variant="secondary" instance="ds-open-essays" layout="fill" onClick={onEssays}>
                    Ensayos{essaysCount > 0 ? ` · ${essaysCount}` : ''}
                </Button>
            </div>
        </div>
    );
...
function EditBody({
    ...
    visibleProperties: StudioElement['properties'];
    canEdit: boolean;
    reviewing: boolean;
    frozen: boolean;
    inherited: boolean;
    ...
    return (
        <>
            <div className="min-h-0 flex-1 overflow-auto p-ds-4 space-y-ds-4">
                ...
                <p className="m-0 text-[14px] font-bold text-ds-texto">{style.ripple}</p>
                {hint ? <p className="m-0 text-[14px] text-ds-texto">{hint}</p> : null}
                <p className="m-0 text-[14px] text-ds-texto">{humanSummary(element)}</p>
                {style.shapeShared ? (
                    <p className="m-0 text-[12px] text-ds-texto-tenue">
                        La forma es la de todos los botones. El selector de opciones y el cierre del modal no
                        son botones.
                    </p>
                ) : null}
                {warning ? (
                ...
                {style.decided ? (
                    <Notice instance="ds-locked" variant="info" title="Ya decidido">
                        {style.decidedCopy}
                    </Notice>
                ) : null}
                {visibleProperties.length > 0 && (canEdit || reviewing || !frozen) ? (
                    <VisualPropertyList
    ...
    return (
        <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 space-y-ds-6 overflow-auto p-ds-4">
                ...
                <p className="m-0 text-[20px] font-black uppercase tracking-wide">{style.title}</p>
    ...
    return (
        <div className="min-h-0 flex-1 space-y-ds-6 overflow-auto p-ds-4">
            <Notice instance="ds-done" variant="positive" title="Diseño actualizado">
                {style.title}. El nuevo diseño ya es oficial.
            </Notice>
            {applyResult ? (

...
import { ChefHat, Search, BookOpen, Package, TrendingUp, Settings, Home, X, Plus, Trash2 } from 'lucide-react';
...
export default function RecipesPage() {
    ...
    return (
        <div className="min-h-screen flex" style={{ backgroundColor: '#5B8FB9' }}>
            ...
            {/* Sidebar */}
            <aside className="w-20 flex flex-col items-center py-6 space-y-6 shadow-2xl z-20 sticky top-0 h-screen" style={{ background: 'linear-gradient(to bottom, #4A7A9A, #36606F)' }}>
                <Link href="/" className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform mb-2">
                    <ChefHat className="w-6 h-6 text-[#3F51B5]" />
                </Link>
                <nav className="flex flex-col space-y-4">
                    <SidebarIcon icon={<Home className="w-5 h-5" />} href="/" />
                    <SidebarIcon icon={<BookOpen className="w-5 h-5" />} href="/recipes" active />
                    <SidebarIcon icon={<Package className="w-5 h-5" />} href="/ingredients" />
                    <SidebarIcon icon={<TrendingUp className="w-5 h-5" />} href="/dashboard" />
            ...
            <main className="flex-1 p-8">
                ...
                <div className="mb-8 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        ...
                        <button onClick={() => { setSelectionMode(!selectionMode); setSelectedIds([]); }} className={`ml-auto px-4 py-2 rounded-xl text-xs font-bold border transition-all ${selectionMode ? 'bg-red-500 text-white border-red-600' : 'bg-white/30 text-white border-white/40 hover:bg-white/40'}`}>{selectionMode ? 'CANCELAR SELECCIÓN' : 'SELECCIONAR'}</button>
                    </div>
                </div>

                {/* GRID DE RECETAS: MÁS GAP (gap-6) */}
                {!loading && (
                        ...
                        {filteredRecipes.map((recipe) => (
                            <div key={recipe.id} className="group relative">
                                ...
                                <Link href={`/recipes/${recipe.id}`} className="block h-full" onClick={(e) => selectionMode && (e.preventDefault(), toggleSelection(recipe.id))}>
                                    <div className={`bg-white p-2 rounded-xl shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-1 flex flex-col h-full ${selectedIds.includes(recipe.id) ? 'ring-4 ring-[#5E35B1] scale-95' : ''}`}>

                                        {/* IMAGEN - SIN BORDES Y FONDO BLANCO */}
                ...
                {selectedIds.length > 0 && (
                    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#5E35B1] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-4 z-50 animate-in slide-in-from-bottom-4">
                        ...
                        <button onClick={handleBulkDelete} disabled={isDeleting} className="flex items-center gap-1 hover:text-red-200 transition-colors">
                            ...
                            <span className="text-xs font-black uppercase">{isDeleting ? '...' : 'Eliminar'}</span>
                        </button>
            ...
            <CreateModal
                ...
                addIngredientToRecipe={() => setNewRecipe({ ...newRecipe, ingredients: [...newRecipe.ingredients, { ingredient_id: '', quantity: 0, unit: 'kg' }] })}
...
function SidebarIcon({ icon, active = false, href }: { icon: React.ReactNode; active?: boolean; href?: string }) {
    const style = `w-10 h-10 rounded-xl flex items-center justify-center transition-all ${active ? 'bg-white text-[#3F51B5] shadow-lg' : 'text-white/60 hover:text-white hover:bg-white/10'}`;
    return href ? <Link href={href} className={style}>{icon}</Link> : <button className={style}>{icon}</button>;
}
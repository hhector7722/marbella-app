...
export default function ActividadesClient() {
  ...
  return (
    <div className="space-y-3 month-cal-shell">
      <PageHeader
        title="Actividades"
        description={getMonthLabel(viewMonth)}
        actions={
          <PageActions>
            <Button
              type="button"
            ...
            {isMaster ? (
              <Button variant="icon" asChild>
                <Link href="/staff/actividades/gestion" aria-label="Gestionar actividades">
                  <Settings className="size-5" strokeWidth={1.5} aria-hidden />
                </Link>
              </Button>
            ) : null}
            <Button variant="outline" asChild className="min-h-12 px-3 text-xs">
      ...
      <div className="overflow-hidden rounded-2xl border border-mds-border bg-mds-surface shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-none month-cal-card">
          ...
          {loading ? (
            ...
            <div 
              className="flex flex-col gap-1 bg-mds-muted-surface/50 px-[1.5%] pt-2 pb-1 touch-pan-y month-cal-body"

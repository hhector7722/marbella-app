...
import { Button, LoadingBlock, PageActions, PageHeader } from '@/components/mds';
import {
  prepareReviewAction,
  fetchVenuesAction,
...
export default function RevisionClient() {
  ...
  return (
    <div className="space-y-3 pb-6">
      <PageHeader
        title={dateStr ? format(parseLocalSafe(dateStr), 'EEEE d MMMM yyyy', { locale: es }) : 'Revisión'}
        description="Revisión e importación de actividades"
        actions={
          <PageActions>
            <Button type="button" variant="icon" onClick={handleCancel} aria-label="Tornar">
              <ChevronLeft className="size-5" strokeWidth={2.5} aria-hidden />
            </Button>
            {loadedFromDb && filePath ? (
              <Button type="button" variant="outline" onClick={() => void loadData(true)} className="gap-2 text-xs">
      ...
      <div className="mx-auto max-w-5xl lg:max-w-6xl">
        <div className="overflow-hidden rounded-2xl border border-mds-border bg-mds-surface shadow-sm">
                  ...
                  <div key={i} className="py-4 flex flex-col gap-3 lg:grid lg:grid-cols-[2.5rem_minmax(14rem,1.4fr)_minmax(16rem,1.2fr)_5.5rem_4rem_5.5rem_minmax(10rem,1fr)_2.5rem] lg:gap-3 lg:items-start lg:px-6 lg:py-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 lg:contents">
                      ...
                      <input
                        ...
                        className="flex-1 rounded-md border border-zinc-200 px-2 py-1.5 text-sm focus:border-mds-primary focus:outline-none min-w-[120px] lg:min-h-12 lg:w-full lg:px-3 lg:text-sm"
                          ...
                          {occ.form_start_time ? (
                            <>
                              <label className="flex items-center gap-1 text-[11px] cursor-pointer">
                                <input type="radio" checked={occ.preferred_start_time !== 'form'} onChange={() => updateOccupation(i, 'preferred_start_time', 'pdf')} className="accent-mds-primary" />
                                PDF:
                                <input type="time" value={occ.start_time} onChange={(e) => updateOccupation(i, 'start_time', e.target.value)} className="w-[70px] rounded-md border border-zinc-200 px-1 py-0.5 focus:border-mds-primary focus:outline-none lg:min-h-10 lg:w-[88px]" />
                              </label>
                              <label className="flex items-center gap-1 text-[11px] cursor-pointer bg-mds-primary/10 px-1.5 py-0.5 rounded text-mds-primary">
                                <input type="radio" checked={occ.preferred_start_time === 'form'} onChange={() => updateOccupation(i, 'preferred_start_time', 'form')} className="accent-mds-primary" />
                                Reporte: <span className="font-bold">{occ.form_start_time.substring(0, 5)}</span>
                              </label>
                            </>
                          ) : (
                            <input type="time" value={occ.start_time} onChange={(e) => updateOccupation(i, 'start_time', e.target.value)} className="w-[75px] rounded-md border border-zinc-200 px-1 py-0.5 focus:border-mds-primary focus:outline-none text-sm lg:min-h-10 lg:w-[96px]" />
                          )}
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {occ.form_end_time ? (
                            <>
                              <label className="flex items-center gap-1 text-[11px] cursor-pointer">
                                <input type="radio" checked={occ.preferred_end_time !== 'form'} onChange={() => updateOccupation(i, 'preferred_end_time', 'pdf')} className="accent-mds-primary" />
                                PDF:
                                <input type="time" value={occ.end_time} onChange={(e) => updateOccupation(i, 'end_time', e.target.value)} className="w-[70px] rounded-md border border-zinc-200 px-1 py-0.5 focus:border-mds-primary focus:outline-none lg:min-h-10 lg:w-[88px]" />
                              </label>
                              <label className="flex items-center gap-1 text-[11px] cursor-pointer bg-mds-primary/10 px-1.5 py-0.5 rounded text-mds-primary">
                                <input type="radio" checked={occ.preferred_end_time === 'form'} onChange={() => updateOccupation(i, 'preferred_end_time', 'form')} className="accent-mds-primary" />
                                Reporte: <span className="font-bold">{occ.form_end_time.substring(0, 5)}</span>
                              </label>
                            </>
                          ) : (
                            <input type="time" value={occ.end_time} onChange={(e) => updateOccupation(i, 'end_time', e.target.value)} className="w-[75px] rounded-md border border-zinc-200 px-1 py-0.5 focus:border-mds-primary focus:outline-none text-sm lg:min-h-10 lg:w-[96px]" />
                          )}
                        </div>
                      </div>

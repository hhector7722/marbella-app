    ...
    return (
        <div className="space-y-3">
            <PageHeader
                ...
                actions={
                    <PageActions>
                        <Button type="button" variant="icon" onClick={prevMonth} aria-label="Mes anterior">
                            <ChevronLeft className="size-5" aria-hidden />
                        </Button>
                        <Button
                            type="button"
                            variant="outline"
                            className="min-h-12 px-3 text-[11px] uppercase tracking-widest"
                            onClick={() => {
                        ...
                        <Button type="button" variant="icon" onClick={nextMonth} aria-label="Mes siguiente">
                            ...
                            {isManager && (
                                <div className="relative">
                                    <Button
                                        type="button"
                                        variant={viewingOther && !isPlantilla ? 'secondary' : 'outline'}
                                        onClick={() => setShowEmployeeDropdown(true)}
                                        className={cn(
                                            'min-h-12 gap-1.5 px-3 text-[10px] font-black uppercase tracking-widest',
                                            isPlantilla && 'border-0 bg-transparent shadow-none hover:bg-mds-muted-surface'
                                        )}
                                    >
                                        <span className="max-w-[120px] md:max-w-[160px] truncate">{headerLabel}</span>
                                        <ChevronDown size={10} className="opacity-40 shrink-0" aria-hidden />
                                    </Button>
                                    {!isPlantilla && selectedEmployeeId && (
                                        <button
                                            ...
                                            className="absolute -top-1.5 -right-1.5 w-4.5 h-4.5 bg-mds-danger text-white rounded-full flex items-center justify-center shadow-lg hover:opacity-90 transition-colors z-30 border-2 border-mds-surface"
            ...
            <div className="overflow-hidden rounded-2xl border border-mds-border bg-mds-surface shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {loading ? (
                        ...
                        plantillaWeeksData.length === 0 ? (
                            <EmptyState
                                ...
                                description="Sin datos de asistencia para el periodo seleccionado."
                                className="py-16"
                            />
                        ) : (
            ...
            {showMonthPicker && (
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[150] p-4 animate-in fade-in duration-200" onClick={() => setShowMonthPicker(false)}>
                    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="bg-mds-primary px-8 py-6 flex justify-between items-center text-mds-primary-foreground shrink-0">
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                isSelected ? "bg-mds-primary border-mds-primary text-mds-primary-foreground shadow-lg" : "bg-mds-muted-surface border-transparent text-mds-muted hover:border-mds-primary/20 hover:text-mds-foreground hover:bg-mds-primary/5"
                                            )}
                                        >

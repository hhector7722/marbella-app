        ...
        <div className="flex flex-col h-full overflow-hidden">
                    ...
                    {DENOMINATIONS.map(denom => (
                        <div key={denom} className="grid grid-cols-[80px_1fr_1fr] items-center gap-1.5 p-1.5 bg-white rounded-xl border border-gray-100">
                            ...
                            <div className="flex flex-col items-center gap-0.5">
                                ...
                                <input type="number" min="0" value={inCounts[denom] || ''} onChange={(e) => setInCounts(p => ({ ...p, [denom]: parseInt(e.target.value) || 0 }))} placeholder="0" className="w-full bg-emerald-50/30 border border-emerald-100 rounded-lg p-1 text-center font-black text-emerald-600 outline-none text-xs" />
                            </div>
                            <div className="flex flex-col items-center gap-0.5">
                                <span className="text-[7px] font-black text-rose-400 uppercase tracking-widest leading-none">SALE</span>
                                <input type="number" min="0" value={outCounts[denom] || ''} onChange={(e) => setOutCounts(p => ({ ...p, [denom]: parseInt(e.target.value) || 0 }))} placeholder="0" className={cn("w-full bg-rose-50/30 border rounded-lg p-1 text-center font-black outline-none text-xs transition-all", (outCounts[denom] || 0) > (availableStock[denom] || 0) ? "border-rose-400 text-rose-700" : "border-rose-100 focus:border-rose-300 text-rose-600")} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="p-3 bg-white border-t gap-2.5 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] shrink-0 space-y-3">
                <div className="flex items-center justify-between px-6 bg-gray-50 p-2.5 rounded-2xl border border-gray-100">
                    ...
                    <div className={cn("px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-sm", isBalanced ? (Object.entries(outCounts).some(([d, q]) => q > (availableStock[Number(d)] || 0)) ? "bg-rose-100 text-rose-700" : "bg-emerald-500 text-white") : "bg-rose-100 text-rose-700")}>{isBalanced ? (Object.entries(outCounts).some(([d, q]) => q > (availableStock[Number(d)] || 0)) ? "Falta Stock" : "Equilibrado") : `${Math.abs(totalIn - totalOut).toFixed(2)}€ Dif.`}</div>
                ...
                <div className="flex gap-2">
                    <button onClick={onCancel} className="flex-1 py-3 text-gray-500 font-black uppercase tracking-widest text-[9px] hover:bg-gray-100 rounded-xl transition-all active:scale-95">Cancelar</button>
                    <button onClick={() => onSubmit(totalIn, inCounts, outCounts as any)} disabled={!isBalanced || Object.entries(outCounts).some(([d, q]) => q > (availableStock[Number(d)] || 0))} className={cn("flex-1 py-3 text-white font-black uppercase tracking-widest text-[9px] rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95", (isBalanced && !Object.entries(outCounts).some(([d, q]) => q > (availableStock[Number(d)] || 0))) ? "bg-[#36606F] shadow-[#36606F]/20" : "bg-gray-300 cursor-not-allowed shadow-none opacity-50")}><ArrowRightLeft size={16} strokeWidth={3} />Confirmar Cambio</button>
                </div>
            </div>
        </div>
    );
                            ...
                            <>
                                ...
                                <div className="p-4 grid grid-cols-2 gap-4">
                                    {selectedBox?.type === 'change' ? (
                                        <>
                                            <button onClick={() => setCashModalMode('in')} className="bg-transparent border-0 hover:bg-emerald-50/50 p-6 rounded-2xl flex flex-col items-center gap-2 transition-all group active:scale-95"><div className="w-12 h-12 mb-1"><Image src="/icons/in.png" alt="Entrada" width={48} height={48} className="w-full h-full object-contain" /></div><span className="font-black text-zinc-900">Entrada</span></button>
                                            <button onClick={async () => { const { data } = await supabase.from('cash_box_inventory').select('*').eq('box_id', selectedBox.id).gt('quantity', 0); const initial: any = {}; data?.forEach(d => initial[d.denomination] = d.quantity); setBoxInventoryMap(initial); setBoxInventory(data || []); setCashModalMode('out'); }} className="bg-transparent border-0 hover:bg-rose-50/50 p-6 rounded-2xl flex flex-col items-center gap-2 transition-all group active:scale-95"><div className="w-12 h-12 mb-1"><Image src="/icons/out.png" alt="Salida" width={48} height={48} className="w-full h-full object-contain" /></div><span className="font-black text-zinc-900">Salida</span></button>
            ...
            {isStaffModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setIsStaffModalOpen(false)}>
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                        <div className="bg-[#36606F] px-8 py-4 flex justify-between items-center text-white shrink-0"><div><h3 className="text-lg font-black uppercase tracking-wider leading-none">Plantilla</h3><p className="text-white/50 text-[10px] font-black uppercase tracking-[0.2em] mt-1 italic">Seleccionar Empleado ({allEmployees.length})</p></div><button onClick={() => setIsStaffModalOpen(false)} className="w-10 h-10 flex items-center justify-center bg-white/10 rounded-xl hover:bg-white/20 transition-all text-white active:scale-90"><X size={20} strokeWidth={3} /></button></div>
                        <div className="p-4 bg-gray-50/30"><div className="grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto no-scrollbar pb-2 pt-2">{allEmployees.map((emp) => (<button key={emp.id} onClick={() => router.push(`/profile?id=${emp.id}`)} className="bg-transparent p-4 rounded-2xl border-0 hover:bg-blue-50/50 transition-all active:scale-95 flex flex-row items-center gap-4 group h-20"><div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-lg font-black text-[#5B8FB9] shadow-inner group-hover:bg-[#5B8FB9] group-hover:text-white transition-colors capitalize shrink-0">{emp.first_name.substring(0, 1)}</div><span className="font-black text-sm text-gray-700 text-left capitalize truncate flex-1 leading-none">{emp.first_name}</span></button>))}</div></div>
                    </div>
                </div>
            )}
            {isProductModalOpen && (

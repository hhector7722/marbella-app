...
const StaffOvertimeRow = memo(({
    ...
    <div onClick={onClick} className="flex items-center justify-between p-1.5 md:p-2 bg-white/60 rounded-xl border border-purple-100/30 cursor-pointer hover:bg-white transition-colors group">
        <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-gray-700 capitalize group-hover:text-purple-700 transition-colors leading-none">
                {staff.name}
            </span>
        </div>
        <div className="flex items-center gap-2.5">
            <span className="text-[10px] font-black text-gray-800">{staff.amount > 0.05 ? `${staff.amount.toFixed(0)}€` : " "}</span>

            <div className="flex items-center bg-gray-100/50 rounded-full h-6 px-1 gap-1">
                {/* Toggle Paid Status */}
                <button
...
const WeekOvertimeCard = memo(({
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden transition-all">
            <button onClick={() => onToggleWeek(week.weekId)} className="w-full p-1.5 md:p-2 flex items-center justify-between text-left group transition-colors hover:bg-gray-50/50">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                        <h4 className="text-[12px] md:text-[13px] font-black text-gray-900">Sem {getISOWeek(new Date(week.weekId))}</h4>
                        <span className="font-light mx-0.5 text-gray-300">•</span>
                        <p className="text-[9px] font-bold uppercase pt-0.5 text-gray-500">
                            {format(new Date(week.weekId), "d MMM", { locale: es })} - {format(addDays(new Date(week.weekId), 6), "d MMM", { locale: es })}
                        </p>
                    </div>
                </div>
                <div className="text-right flex items-center gap-3">
                    <span className="text-base font-black text-gray-900">{week.total > 0.05 ? `${week.total.toFixed(0)}€` : " "}</span>
                </div>
            </button>
            {week.expanded && (
                <div className="px-2 pb-2 pt-0.5 space-y-1.5 animate-in slide-in-from-top-2 duration-300">
...
const AdminDashboardView = ({ initialData }: { initialData?: any }) => {
    ...
    return (
        <div className="pt-0 md:pt-0.5 animate-in fade-in duration-500">
            <div className="px-3 md:px-6 w-full max-w-2xl lg:max-w-[650px] mx-auto space-y-2.5 md:space-y-3.5 mt-0 md:mt-1">
                <div className="flex flex-col gap-3 md:gap-4 items-center">
                    {/* 1. VENTAS */}
                    <div className="w-full bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden">
                        <div className="bg-[#36606F] px-4 py-2 md:py-3 flex justify-between items-center text-white shrink-0 relative">
                            <div className="flex items-center gap-2">
                                <h3 className="text-sm md:text-base font-black uppercase tracking-wider leading-none">Ventas</h3>
                            </div>
                            <div className="absolute left-1/2 -translate-x-1/2 scale-100 md:scale-125"><LiveClock /></div>
                            <div className="flex items-center gap-3">
                                <Link href="/dashboard/history" className="text-[10px] md:text-xs font-black pointer-events-auto hover:text-white/80 transition-colors uppercase tracking-widest">
                                    Ver más
                                </Link>
                            </div>
                        </div>
                        <div className="p-2 md:p-3 grid grid-cols-3 gap-y-3 gap-x-4 flex-1 items-center">
                            <div className="flex flex-col items-center justify-center text-center">
                                <PremiumCountUp value={liveTickets.total} suffix="€" decimals={2} className="text-xl md:text-2xl font-black text-black leading-none" />
                                <span className="text-[8px] md:text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1.5">Ventas</span>
                            </div>
                            <div className="flex flex-col items-center justify-center text-center">
                                <PremiumCountUp value={liveTickets.total > 0 ? liveTickets.total / 1.10 : 0} suffix="€" decimals={2} className="text-xl md:text-2xl font-black text-emerald-600 leading-none" />
                                <span className="text-[8px] md:text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1.5">Venta Neta</span>
                            </div>
                            <div className="flex flex-col items-center justify-center text-center">
                                <PremiumCountUp value={liveTickets.count > 0 ? liveTickets.total / liveTickets.count : 0} suffix="€" decimals={2} className="text-xl md:text-2xl font-black text-blue-600 leading-none" />
                                <span className="text-[8px] md:text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1.5">Ticket Medio</span>
                            </div>
                        </div>
                    </div>
                    {/* 2. HORAS EXTRAS */}
                    <div className="w-full bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden self-start">
                        <div className="bg-purple-600 px-4 py-1.5 md:py-2 flex justify-between items-center text-white shrink-0">
                            <h2 className="text-[11px] md:text-xs font-black uppercase tracking-wider leading-none">Horas Extras</h2>
                            <div className="flex items-center gap-4">
                                {currentUserEmail === 'hhector7722@gmail.com' && (
                                    <button
                                        onClick={handleRecalculate}
                                        disabled={isRecalculating}
                                        className="text-[10px] md:text-xs font-black hover:text-white/80 transition-colors uppercase tracking-widest flex items-center gap-2 disabled:opacity-50"
                                    >
                                        {isRecalculating ? <LoadingSpinner size="sm" /> : <RotateCcw size={14} />}
                                        Recalcular
                                    </button>
                                )}
                                <Link href="/dashboard/overtime" className="text-[10px] md:text-xs font-black hover:text-white/80 transition-colors uppercase tracking-widest">Ver más</Link>
                            </div>
                        </div>
                        <div className="p-2 md:p-2.5 space-y-2 max-h-[300px] overflow-y-auto no-scrollbar pr-1">
                            {overtimeData.length === 0 ? (
                                ...
                                overtimeData.slice(0, 2).map((week) => (
                                    <div key={week.weekId} className="scale-100 origin-top">
                                        <WeekOvertimeCard
                                            week={week}
                    ...
                    {/* 3. CAJA OPERATIVA */}
                    <div className="w-full bg-white rounded-2xl p-2 md:p-3 shadow-xl flex flex-col">
                        {boxes.filter(b => b.type === 'operational').map(box => (
                            <div key={box.id} className="flex flex-col h-full">
                                <div className="flex flex-row gap-2 md:gap-2.5 mb-2 md:mb-2.5">
                                    <button
                                        onClick={() => router.push('/dashboard/movements')}
                                        className="grow-[0.8] basis-0 px-3 py-1.5 md:py-2 rounded-xl bg-emerald-600 shadow-lg hover:bg-emerald-700 transition-all cursor-pointer flex flex-row items-center justify-between text-white active:scale-95"
                                    >
                                        <div className="flex flex-col items-start leading-none gap-1">
                                            <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] opacity-80">Caja Inicial</span>
                                            <span className="text-base md:text-lg font-black">{Math.abs(actualBalance) > 0.005 ? `${actualBalance.toFixed(2)}€` : " "}</span>
                                        </div>
                                    </button>

                                    <div className="flex-[1.8] basis-0 grid grid-cols-3 gap-1 md:gap-2">
                                        <button
                                            onClick={() => openTreasuryModal(box, 'in')}
                                            className="bg-transparent hover:bg-emerald-50/50 p-1.5 rounded-xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 group"
                                        >
                                            ...
                                            <span className="text-[8px] md:text-[9px] font-black text-zinc-900 uppercase tracking-widest leading-none">Entrada</span>
                                        </button>
                                        <button
                                            onClick={() => openTreasuryModal(box, 'out')}
                                            className="bg-transparent hover:bg-rose-50/50 p-1.5 rounded-xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 group"
                                        >
                                            ...
                                            <span className="text-[8px] md:text-[9px] font-black text-zinc-900 uppercase tracking-widest leading-none">Salida</span>
                                        </button>
                                        <button
                                            onClick={() => openTreasuryModal(box, 'audit')}
                                            className="bg-transparent hover:bg-orange-50/50 p-1.5 rounded-xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 group"
                                        >
                                            <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-orange-500 rounded-full shadow-sm group-hover:scale-110 transition-transform">
                                                <RefreshCw size={14} strokeWidth={3} className="text-white" />
                                            </div>
                                            <span className="text-[8px] md:text-[9px] font-black text-zinc-900 uppercase tracking-widest leading-none">Arqueo</span>
                                        </button>
                                    </div>
                                </div>
                                <div className="flex flex-col flex-1 min-h-0">
                                    <div className="flex items-center px-2 mb-2">
                                        {Math.abs(difference || 0) < 0.01 ? (
                                            <span className="text-[10px] md:text-xs font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1"><Check className="w-4 h-4" /> Caja Cuadrada</span>
                                        ) : (
                                            <span className={cn(
                                                "text-[10px] md:text-sm font-black uppercase tracking-widest flex items-center gap-2",
                                                (difference || 0) < 0 ? "text-rose-500" : "text-emerald-500"
                                            )}>
                                                <AlertTriangle className="w-4 h-4 md:w-5 md:h-5" strokeWidth={3} />
                                                Diferencia: {Math.abs(difference || 0) > 0.005 ? `${(difference || 0) > 0 ? '+' : ''}${(difference || 0).toFixed(2)}€` : " "}
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex justify-between items-center px-1 mb-1.5">
                                        <button onClick={() => setIsMovementsExpanded(!isMovementsExpanded)} className="flex items-center gap-1.5 text-[8.5px] md:text-[9px] font-black text-zinc-400 uppercase tracking-widest hover:text-zinc-600 transition-colors">
                                            Movimientos <ChevronDown className={cn("w-3 h-3 transition-transform duration-200", isMovementsExpanded && "rotate-180")} />
                                        </button>
                                        <Link href="/dashboard/movements" className="text-[8.5px] md:text-[9px] font-black text-[#5B8FB9] bg-gray-50/50 px-2.5 py-1.5 rounded-full hover:bg-gray-100 transition-all flex items-center gap-1 uppercase">Ver más <ArrowRight className="w-2.5 h-2.5" /></Link>
                                    </div>
                                    <div className={cn("overflow-hidden transition-all duration-300", isMovementsExpanded ? "flex-1 opacity-100" : "h-0 opacity-0")}>
                                        <div className="space-y-3 py-2 max-h-[300px] overflow-y-auto no-scrollbar">
                                            {boxMovements.length === 0 && <p className="text-[10px] text-gray-300 italic px-1 text-center py-6">Sin historial reciente</p>}
                                            {boxMovements.map(mov => (
                                                <div key={mov.id} className="flex justify-between items-center text-[11px] md:text-sm bg-gray-50 p-4 rounded-2xl border border-gray-100/50">
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        {mov.type === 'OUT' ? <ArrowUpRight className="w-4 h-4 text-rose-400 shrink-0" /> : <ArrowDownLeft className="w-4 h-4 text-emerald-500 shrink-0" />}
                                                        <span className="truncate max-w-[200px] text-gray-600 font-medium">{mov.notes || 'Sin nota'}</span>
                                                    </div>
                                                    <span className={cn("font-black", mov.type === 'OUT' ? 'text-rose-500' : 'text-emerald-600')}>
                                                        {mov.type === 'OUT' ? '-' : '+'}{mov.amount > 0.005 ? `${mov.amount.toFixed(2)}€` : " "}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 pb-2">
                        {/* 4. CAJAS CAMBIO */}
                        <div className="bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden md:col-span-7 min-h-[120px] md:min-h-[130px]">
                            <div className="bg-[#36606F] px-4 py-1.5 flex items-center text-white shrink-0">
                                <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest leading-none">Cajas Cambio</h3>
                            </div>
                            <div className="p-2 md:p-2.5 flex-1 flex flex-col justify-around gap-1.5 md:gap-2">
                                {boxes.filter(b => b.type === 'change').slice(0, 2).map((box, idx) => {
                                    const diff = box.current_balance - 300;
                                    const isOk = Math.abs(diff) < 0.01;

                                    return (
                                        <div key={box.id} className="flex flex-row gap-3 items-center">
                                            <div className="flex-1 flex flex-col items-start leading-none gap-1 px-1">
                                                <span className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Cambio {idx + 1}</span>
                                                <span className="text-base md:text-xl font-black text-zinc-800">{box.current_balance > 0.005 ? `${box.current_balance.toFixed(2)}€` : " "}</span>
                                                {!isOk && (
                                                    <span className={cn("text-[8px] md:text-[10px] font-black", diff < 0 ? "text-rose-500" : "text-emerald-600")}>
                                                        {diff > 0.005 ? `+${diff.toFixed(2)}€` : `${diff.toFixed(2)}€`}
                                                    </span>
                                                )}
                                            </div>

                                            <div className="flex gap-2">
                                                <button onClick={() => { setSelectedBox(box); setCashModalMode('swap'); }} className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center bg-blue-500 rounded-lg shadow-sm hover:scale-110 transition-transform active:scale-95 text-white">
                                                    <ArrowRightLeft size={16} strokeWidth={4} />
                                                </button>
                                                <button onClick={() => openTreasuryModal(box, 'audit')} className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center bg-orange-500 rounded-lg shadow-sm hover:scale-110 transition-transform active:scale-95 text-white">
                        ...
                        <div className="md:col-span-5 grid grid-cols-2 gap-2">
                            {[
                                ...
                                <button
                                    ...
                                    className="bg-white rounded-2xl p-2 shadow-sm flex flex-col items-center justify-center gap-1.5 active:scale-95 transition-all group aspect-square w-full h-full"
                                >
                                    <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center transition-transform group-hover:scale-110 overflow-hidden">
                                        <Image src={card.img} alt={card.title} width={48} height={48} priority={true} className="w-full h-full object-contain" />
                                    </div>
                                    <span className="text-[9px] md:text-[10px] font-black text-gray-800 uppercase tracking-wider text-center line-clamp-2 leading-tight px-0.5">
                                        {card.title}
                                    </span>
                                </button>
                            ))}
                    ...
                    <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-4 duration-300" onClick={e => e.stopPropagation()}>
                        <div className="bg-[#36606F] px-8 py-4 flex justify-between items-center text-white shrink-0">
                            <div>
                                ...
                                <p className="text-white/50 text-[10px] font-black uppercase tracking-[0.2em] mt-1 italic">Gestión de Artículos</p>
                            </div>

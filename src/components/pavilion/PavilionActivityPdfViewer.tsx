...
import { computeFitScale, renderPdfPageHiDpi } from '@/lib/pdf/hidpi-render';
...
export function PavilionActivityPdfViewer({ url, className }: PavilionActivityPdfViewerProps) {
  ...
  const renderAllPages = useCallback(async () => {
    ...
    if (!pdfDoc || !host || containerWidth <= 0) return;
    ...
    try {
      ...
      const fitScale = computeFitScale(page1.getViewport({ scale: 1 }).width, containerWidth);
      ...
      for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum++) {
        ...
        await renderPdfPageHiDpi(page, canvas, renderScale);
  ...
  useEffect(() => {
    if (loadingDoc || containerWidth <= 0 || !pdfDocRef.current) return;
  ...
  return (
    <div className={cn('flex flex-col min-h-0', className)}>
      <div className="relative flex min-h-0 w-full max-h-[70vh]">
        ...
        <div
          ref={scrollRef}
          className="w-full min-h-0 max-h-[70vh] overflow-auto overscroll-contain bg-zinc-50/60 p-3"
        ...
        >
          <div ref={pagesHostRef} className="flex flex-col items-center min-w-min" />
        </div>
      </div>
    </div>
  );

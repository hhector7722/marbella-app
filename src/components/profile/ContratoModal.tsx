...
import { getEmployeeDocuments } from '@/app/actions/profile';
...
export default function ContratoModal({ isOpen, onClose, userId }: ContratoModalProps) {
    ...
    useEffect(() => {
        ...
        const fetchDocs = async () => {
            ...
            try {
                ...
                const contracts = (data || []).filter((d: any) => d.type === 'contract');
                setDocs(contracts);
            } catch {
                setDocs([]);
            } finally {
                        ...
                        <ul className="space-y-2">
                            {docs.map((row) => (
                                <li key={row.id} className="min-h-[60px] flex items-center justify-between gap-3 p-4 rounded-2xl border border-zinc-100 bg-white hover:bg-zinc-50/80">
                                    <p className="font-semibold text-zinc-800 truncate flex-1 min-w-0">{row.file_name || 'Contrato'}</p>

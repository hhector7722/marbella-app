...
function toHhMm(raw: string | null | undefined): string {
    ...
    if (!v) return '';
    // Hora civil «HH:mm» / «HH:mm:ss» (p. ej. columnas `time` de Postgres):
    // se normaliza sin pasar por Date para no reinterpretarla por zona horaria.
    const civil = /^\s*(\d{1,2}):(\d{2})(?::\d{1,2})?\s*$/.exec(v);
    if (civil) return `${civil[1].padStart(2, '0')}:${civil[2]}`;
...
export const ScheduleDayEditor = forwardRef<ScheduleDayEditorHandle, ScheduleDayEditorProps>(function ScheduleDayEditor(
    ...
    const buildPersistPayload = () => {
        ...
        const toStartIso = (time: string, label: string): string => {
            // Normaliza la hora civil a «HH:mm» antes de anclarla al día local.
            const hhmm = toHhMm(time);
            const parsed = new Date(`${date}T${hhmm}:00`);
            if (!hhmm || Number.isNaN(parsed.getTime())) {
        ...
        >
            ...
            <div className="flex flex-col flex-1 min-h-0 overflow-hidden day-modal-body">
                ...
                <div data-element="schedule-shift-table" className="rounded-2xl border border-zinc-200/60 shadow-[0_1px_2px_rgba(0,0,0,0.05),0_12px_32px_rgba(0,0,0,0.16)] overflow-hidden flex flex-col flex-1 min-h-0">
                    ...
                    <div
                        className="flex flex-col w-full bg-white flex-1 overflow-y-auto min-h-0 day-modal-shift-rows"
                        onTouchStart={(e) => e.stopPropagation()}
                        onTouchMove={(e) => e.stopPropagation()}
                    >

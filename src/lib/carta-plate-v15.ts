...
import plateV15Layout from './carta-plate-v15-layout.json'

export const PLATE_V15_CANVAS = {
  width: plateV15Layout.canvas[0],
  height: plateV15Layout.canvas[1],
} as const
export const PLATE_V15_BASE_PATH = '/images/carta/plato-marbella/v15'

export type PlateV15AssetKey =
  | 'carbonara'
  | 'crema'
  | 'ensalada'
  | 'gazpacho'
...
export type PlateV15Transform = {
  scale: number
  x: number
  y: number
}

export type PlateV15HitBox = {
  left: number
  top: number
  width: number
  height: number
...
export const PLATE_V15_BOWL_STARTERS = new Set<PlateV15AssetKey>([
  'carbonara',
  'crema',
  'ensalada',
  'gazpacho',
...
export const PLATE_V15_TRANSFORMS = plateV15Layout.assets as Readonly<
  Record<PlateV15AssetKey | 'bowl_group', PlateV15Transform>
>
 ...
 */
export const PLATE_V15_HITBOXES: Readonly<Record<PlateV15AssetKey, PlateV15HitBox>> = {
...
const ARTICLE_ASSET: Readonly<Record<number, PlateV15AssetKey>> = {
  138: 'berenjena',
  139: 'gazpacho',
  141: 'pesto',
  142: 'carbonara',
...
const DEFAULT_HITBOX_BY_SLOT: Readonly<Record<PlatoMarbellaSlot, PlateV15HitBox>> = {
...
export function resolvePlateV15Asset(input: {
  id?: string
  label: string
  slot: PlatoMarbellaSlot
}): PlateV15AssetKey | null {
...
export function plateV15AssetPath(key: PlateV15AssetKey): string {
  if (PLATE_V15_BOWL_STARTERS.has(key) || key === 'berenjena') {
    return `${PLATE_V15_BASE_PATH}/entrantes/${key}.png`
  }
  if (key === 'patatas' || key === 'verduras') {
    return `${PLATE_V15_BASE_PATH}/guarniciones/${key}.png`
  }
  return `${PLATE_V15_BASE_PATH}/principales/${key}.png`
}

export function plateV15HitBox(key: PlateV15AssetKey | null, slot: PlatoMarbellaSlot): PlateV15HitBox {
  return key ? PLATE_V15_HITBOXES[key] : DEFAULT_HITBOX_BY_SLOT[slot]
}

export function plateV15LabelPosition(slot: PlatoMarbellaSlot): { left: number; top: number } {

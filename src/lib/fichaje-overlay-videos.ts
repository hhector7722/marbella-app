/**
 * Vídeos de overlay al fichar, por email (entrada/salida).
 * Rutas desde /icons (public/icons/*.mp4).
 */
export const FICHAJE_OVERLAY_VIDEOS: Record<string, { entrada: string; salida: string }> = {
    'guillemruizhomet@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    'hugorubiolarripa@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    'marbellaremote@gmail.com': { entrada: '/icons/giff.mp4', salida: '/icons/cris.mp4' },
    'pacostaguiriguet@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    'pereboladeres@gmail.com': { entrada: '/icons/video-españa.mp4', salida: '/icons/video-españa.mp4' },
    'lurodero04@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    's29valiente@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    'aldoruggeri1512@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
    'hernang6799@gmail.com': { entrada: '/icons/video-españa.mp4', salida: '/icons/video-españa.mp4' },
    'fggutierrez98es@gmail.com': { entrada: '/icons/video-españa.mp4', salida: '/icons/video-españa.mp4' },
    'hhector7722@gmail.com': { entrada: '/icons/video-españa.mp4', salida: '/icons/video-españa.mp4' },
    'mamadou.ndiaye.7611@gmail.com': { entrada: '/icons/mamadou-ndiaye.mp4', salida: '/icons/mamadou-ndiaye.mp4' },
    'jalvez853@gmail.com': { entrada: '/icons/riquelme.mp4', salida: '/icons/riquelme.mp4' },
    'albamasia.opos@gmail.com': { entrada: '/icons/cr7.mp4', salida: '/icons/cris.mp4' },
};

...
export interface NormalizerPeriod {
    year: number;
    /** 0-indexed (0 = enero) */
    month: number;
}
...
/** Alias explícito para la UI. */
export function canSimulateStaffSchedule(
    weeksData: TimesheetWeekData[],
    contract: NormalizerContract,
    todayYmd?: string,
): SimulationResolution {
    return resolveSimulationProfile(weeksData, contract, todayYmd);
}
...
export function normalizeStaffSchedule(
    ...
    const today = todayYmd ?? formatYmd(new Date());
    const resolution = resolveSimulationProfile(weeksData, contract, today);
    if (!resolution.canSimulate) {
        throw new SimulationUnavailableError(resolution.reason ?? INSUFFICIENT_HISTORY_MESSAGE);
    }

    const weeks = cloneWeeks(weeksData);
    const isHector = isMasterDashboardUser(employee.email);
    const weeklyTarget = resolution.contractedHoursWeekly;
    const simBounds = getSimulationBounds(today);
    const refStart = referencePeriodStart(simBounds.start);
    const refEnd = referencePeriodEnd(simBounds, contract.endDate, today);
    ...
    if (isHector) {
        applyHectorBaseline(weeks, employee.userId, contract, today, pattern, simBounds);
        clearHectorWeekends(weeks);
    }

    for (const week of weeks) {
        balanceWeek(week, employee.userId, weeklyTarget, contract, today, isHector, pattern, simBounds);

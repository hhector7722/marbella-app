-- ==============================================================================
-- tip_pool_cents_allocation_no_zero_shares
-- Arregla redondeos por céntimos en get_tip_pool_preview:
-- en lugar de ROUND() por fila (que puede dejar 0.00€ para gente con horas),
-- reparte los céntimos restantes según la parte fraccionaria para mantener
-- consistencia y minimizar celdas "0.00€" cuando hay horas y bote > 0.
-- ==============================================================================
...
BEGIN
    ...
    weekday_remaining AS (
        SELECT
            GREATEST(
                (round(pv.weekday_cash_total * 100)::bigint - SUM(b.weekday_base_cents))::bigint,
                0::bigint
            ) AS remaining_cents
        FROM weekday_base b
        CROSS JOIN pool_values pv
    ...
    weekend_remaining AS (
        SELECT
            GREATEST(
                (round(pv.weekend_cash_total * 100)::bigint - SUM(b.weekend_base_cents))::bigint,
                0::bigint
            ) AS remaining_cents
        FROM weekend_base b
        CROSS JOIN pool_values pv

-- ==============================================================================
-- tip_pool_event_type_filter_adjust_regular_weekend
-- Reglas del cálculo de horas para propinas:
-- Contabilizar solo time_logs con event_type en:
--   - 'adjustment'
--   - 'regular'
--   - 'weekend'
-- o bien event_type = NULL
-- ==============================================================================
...
BEGIN
    ...
    logs AS (
        ...
        WHERE (tl.clock_in AT TIME ZONE 'Europe/Madrid')::date >= p_start_date
          ...
          -- SOLO las reglas pedidas por el usuario
          AND (tl.event_type IS NULL OR tl.event_type IN ('adjustment', 'regular', 'weekend'))
    ),
    hours_by_user AS (
        SELECT
            s.id AS user_id,

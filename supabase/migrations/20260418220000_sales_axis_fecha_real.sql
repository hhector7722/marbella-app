-- Eje cronológico de negocio: fecha_real (recepción en servidor) en Europe/Madrid.
-- La columna `fecha` del TPV queda solo como trazabilidad forense, no para KPIs ni gráficas.
--
-- DROP previo: CREATE OR REPLACE no puede cambiar el tipo de retorno de una función existente (42P13).
...
AS $$
  ...
  WHERE t.fecha_real IS NOT NULL
    AND (timezone('Europe/Madrid', t.fecha_real))::date = target_date;
$$;
...
BEGIN
    RETURN QUERY
    SELECT
        (timezone('Europe/Madrid', t.fecha_real))::date AS fecha,
        EXTRACT(HOUR FROM timezone('Europe/Madrid', t.fecha_real))::int AS hora,
        ROUND(SUM(t.total_documento)::numeric, 2) AS total
    FROM public.tickets_marbella t
    WHERE t.fecha_real IS NOT NULL
      AND (timezone('Europe/Madrid', t.fecha_real))::date >= p_start_date
      AND (timezone('Europe/Madrid', t.fecha_real))::date <= p_end_date
    GROUP BY 1, 2
    ORDER BY 1, 2;
END;
$$;
...
BEGIN
    ...
    WHERE tl.fecha_real IS NOT NULL
      AND (timezone('Europe/Madrid', tl.fecha_real))::date >= p_start_date
      AND (timezone('Europe/Madrid', tl.fecha_real))::date <= p_end_date
...
BEGIN
    ...
    LEFT JOIN public.tickets_marbella t
      ON t.fecha_real IS NOT NULL
     AND (timezone('Europe/Madrid', t.fecha_real))::date = d.d::date
    GROUP BY d.d
    ORDER BY d.d;
END;
$$;

-- kds_orders.nombre_cliente desde radiografia_completa (extractor TPV)

ALTER TABLE public.kds_orders ADD COLUMN IF NOT EXISTS nombre_cliente text;
...
DROP FUNCTION IF EXISTS public.fn_calculate_and_insert_delta(text, text, text, jsonb, text);
...
CREATE OR REPLACE FUNCTION public.fncalcdelta(
  ...
  anombre_cliente text DEFAULT NULL::text
)
RETURNS void
LANGUAGE plpgsql
AS $function$
...
BEGIN
  ...
  v_cli := NULLIF(btrim(COALESCE(anombre_cliente, '')), '');

  IF anombre_cliente IS NOT NULL THEN
    UPDATE public.kds_orders o
    SET nombre_cliente = v_cli
  ...
  IF v_last_order_id IS NULL
    ...
    INSERT INTO public.kds_orders (id_ticket, mesa, notas_comanda, estado, origen, nombre_cliente)
    VALUES (aid, amesa, anotas, 'activa', 'TPV', v_cli)
...
COMMENT ON FUNCTION public.fncalcdelta(text, text, text, jsonb, text, text) IS
  'Delta KDS: FULL OUTER JOIN hashable; batch 120s; nombre_cliente desde TPV.';
...
AS $function$
BEGIN
  PERFORM public.fncalcdelta(p_id_ticket, p_mesa, p_notas_comanda, p_productos, p_numero_documento, p_nombre_cliente);
END;
$function$;

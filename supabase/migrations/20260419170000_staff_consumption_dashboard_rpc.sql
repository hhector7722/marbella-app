-- Dashboard: Consumo personal (resumen calendario + detalle por día)
-- Permisos: solo manager/admin vía helper public.is_manager_or_admin()
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      NULLIF(split_part(sm.reference_doc, '-', 2), '')::uuid AS employee_id,
    ...
    WHERE sm.movement_type = 'WASTE'
      ...
      AND (p_user_id IS NULL OR NULLIF(split_part(sm.reference_doc, '-', 2), '')::uuid = p_user_id)
  ),
  agg AS (

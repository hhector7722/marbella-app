-- 1. Añadir columna 'nombre' a las líneas de ticket para persistir el nombre enviado por el TPV
ALTER TABLE public.ticket_lines_marbella ADD COLUMN IF NOT EXISTS nombre TEXT;

-- 2. Actualizar el RPC de detalle de ticket para priorizar el nombre guardado en la línea
CREATE OR REPLACE FUNCTION public.get_ticket_lines(p_numero_documento TEXT)
RETURNS TABLE (
    cantidad NUMERIC,
...
BEGIN
    ...
    SELECT 
        tl.unidades,
        COALESCE(tl.nombre, a.nombre, 'Producto Desconocido'),
        tl.precio_unidad,
        tl.importe_total
    FROM public.ticket_lines_marbella tl
    LEFT JOIN public.bdp_articulos a ON tl.articulo_id = a.id
    WHERE tl.numero_documento = p_numero_documento
    ORDER BY tl.linea;
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;

-- 3. Actualizar el trigger del KDS para permitir artículos con el prefijo '↳ ' (packs/menús)
...
BEGIN
  ...
  LOOP
    ...
    -- Lógica de filtrado KDS
    v_effective_envia := FALSE;
    
    -- EXCEPCIÓN PARA PACKS: Si el nombre empieza por '↳ ', forzamos su envío al KDS
    IF v_nombre LIKE '↳ %' THEN
      v_effective_envia := TRUE;
    ELSIF v_articulo_id IS NOT NULL THEN

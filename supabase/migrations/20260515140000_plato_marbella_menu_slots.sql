-- Plato Marbella: tramos (entrante/principal/guarnición) y precio único del menú en carta.

begin;

alter table public.digital_menu_overrides
  add column if not exists plato_marbella_slot text;

alter table public.digital_menu_overrides
  add column if not exists plato_marbella_is_menu_price boolean not null default false;

alter table public.digital_menu_overrides
  drop constraint if exists digital_menu_overrides_plato_marbella_slot_check;

alter table public.digital_menu_overrides
  add constraint digital_menu_overrides_plato_marbella_slot_check
  check (
    plato_marbella_slot is null
    or plato_marbella_slot in ('entrante', 'principal', 'guarnicion')
  );

alter table public.digital_menu_overrides
  drop constraint if exists digital_menu_overrides_plato_marbella_slot_price_exclusive;

alter table public.digital_menu_overrides
  add constraint digital_menu_overrides_plato_marbella_slot_price_exclusive
  check (
    not (
      coalesce(plato_marbella_is_menu_price, false) = true
      and plato_marbella_slot is not null
    )
  );
...
comment on view public.v_digital_menu_items is
  'Carta digital: TPV + overrides + portada + i18n + factor porción + Plato Marbella slots.';
...
comment on view public.v_public_menu_items is
  'Carta pública (QR): nombre+precio; fotos en Tapas/Bocadillos/Platos; Plato Marbella slots.';

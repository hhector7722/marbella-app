-- Carta: talla visual de foto por producto (S / M / L).

begin;

alter table public.digital_menu_overrides
  add column if not exists carta_photo_scale text not null default 'm';

alter table public.digital_menu_overrides
  drop constraint if exists digital_menu_overrides_carta_photo_scale_check;

alter table public.digital_menu_overrides
  add constraint digital_menu_overrides_carta_photo_scale_check
  check (carta_photo_scale in ('s', 'm', 'l'));
...
comment on view public.v_digital_menu_items is
  'Carta digital: TPV + overrides + portada + i18n + factor porción + Plato Marbella + talla foto.';
...
comment on view public.v_public_menu_items is
  'Carta pública (QR): nombre+precio; fotos y talla S/M/L en Tapas/Bocadillos/Platos/Bebidas.';

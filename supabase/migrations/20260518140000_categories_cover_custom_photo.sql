-- Portada categoría/subcategoría: imagen libre (URL) además de artículo TPV.
...
alter table public.categories
  add column if not exists cover_photo_scale text;

comment on column public.categories.cover_photo_url is
  'URL pública de imagen de portada (p. ej. Storage). Si no es null/vacío, prevalece sobre cover_articulo_id.';

comment on column public.categories.cover_photo_scale is
  'Talla visual S/M/L solo para cover_photo_url; null si la portada viene de cover_articulo_id.';

alter table public.categories
  drop constraint if exists categories_cover_photo_scale_check;

alter table public.categories
  add constraint categories_cover_photo_scale_check
  check (cover_photo_scale is null or cover_photo_scale in ('s', 'm', 'l'));
...
comment on view public.v_digital_menu_items is
  'Carta digital: TPV + overrides + portada (URL propia o artículo) + i18n + factor porción + Plato Marbella + talla foto.';
...
select
  ...
  case
    when category_parent_name in ('Tapas', 'Bocadillos', 'Platos', 'Bebidas')
    then carta_photo_scale
    else 'm'
  end as carta_photo_scale,
  sort_order,
  category_parent_id,
...
comment on view public.v_public_menu_items is
  'Carta pública (QR): nombre+precio; fotos y talla S/M/L en Tapas/Bocadillos/Platos/Bebidas; portada sección con URL propia o artículo.';

-- get_financial_statement: ventas en base imponible (s/IVA) + bruto TPV informativo (c/IVA)
...
BEGIN
  ...
  -- PY&L (Devengo): Horas extras (coste semanal acumulado en weekly_snapshots).
  SELECT COALESCE(SUM(ws.total_cost), 0)
  INTO v_overtime_total
  FROM public.weekly_snapshots ws
  WHERE ws.week_start >= p_start_date
  ...
  v_meta := jsonb_build_object(
    ...
    'sources', jsonb_build_object(
      ...
      'accrualOvertime', 'weekly_snapshots(total_cost) by week_start in range',

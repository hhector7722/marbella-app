-- get_product_margin_ranking: filtro opcional por rango (fecha_real Madrid, fallback fecha_negocio)

DROP FUNCTION IF EXISTS public.get_product_margin_ranking(int);
...
COMMENT ON FUNCTION public.get_product_margin_ranking(int, date, date) IS
  'Margen por producto TPV con receta; top N por unidades en rango opcional (fecha_real Madrid o fecha_negocio). NULL = histórico global.';

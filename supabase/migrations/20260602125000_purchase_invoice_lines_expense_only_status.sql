-- Líneas de albarán que son gasto pero no generan stock (pilas, insecticida, gestoría...).
-- status = 'expense_only' → sin mapped_ingredient_id, sin stock PURCHASE, cuenta como resuelta.

COMMENT ON COLUMN public.purchase_invoice_lines.status IS
  'pending: sin resolver; mapped: vinculada a ingrediente; excluded: portes/ajuste/sin cargo (no requiere ingrediente ni stock); expense_only: gasto sin stock (no requiere ingrediente ni stock).';

-- Auto-mapeo: no tocar líneas excluidas ni "expense_only" (idempotente).
...
BEGIN
  FOR rec IN
    ...
    WHERE pil.mapped_ingredient_id IS NULL
      AND COALESCE(pil.status, 'pending') NOT IN ('excluded', 'expense_only')

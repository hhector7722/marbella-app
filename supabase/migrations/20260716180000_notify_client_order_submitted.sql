...
-- Notificación in-app al enviar pedido cliente (mismo patrón que reservas → user_notifications).
-- Sin tablas nuevas. Sin push. El type NO es reservation_new para aparecer en NotificationsBell.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    v_items_out := v_items_out || jsonb_build_array(
      jsonb_build_object('product_id', v_pid, 'name', v_name, 'quantity', v_qty, 'unit_price', v_price)
    );
  END LOOP;
  ...
  -- Fecha legible ES: "Viernes 24 julio · 21:30"

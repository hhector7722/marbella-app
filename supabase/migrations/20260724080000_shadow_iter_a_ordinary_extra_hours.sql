-- =================================================================
-- Shadow Convergence Iteración A (única causa raíz):
-- Persistir ordinary_hours / extra_hours en weekly_snapshots.
--
-- Evidencia: SHADOW_VALIDATION_REPORT.md Grupo A.
-- Ubicación: fn_recalc_and_propagate_snapshots (columnas ya existen;
-- el INSERT histórico no las escribía → default 0).
--
-- Fórmula alineada a Hours Engine regime-policy (referencia SSOT):
--   - agosto / manager / fixed: ordinary=0; extra = horas de fichaje (logs)
--   - staff: ordinary = min(postjoin, contrato); extra = max(0, postjoin−contrato) + prejoin
--
-- NO toca: end_date, manager=40 en total_hours, carry, Hours Engine TS.
-- =================================================================
...
begin
  ...
  while v_current_week <= v_end_date loop
    ...
    if extract(month from v_current_week) = 8 then
      ...
      -- HE agosto: ordinary=0; overtime=horas fichadas
      v_ordinary_hours := 0;
      v_extra_hours := v_logs_sum;
    elsif v_role = 'manager' or coalesce(v_is_fixed_salary, false) then
      -- Iteración A: NO elimina manager=40 en total_hours (eso es Iter C).
      -- Desglose canónico HE: ordinary=0; extra = solo fichajes reales.
      ...
      -- HE staff: ordinary = min(horas, contrato); OT = resto; pre-alta → todo OT
...
comment on function public.fn_recalc_and_propagate_snapshots(uuid, date) is
  'Recalc weekly_snapshots. Iter A: persiste ordinary_hours/extra_hours (alineado HE regime).';

-- Backfill SOLO ordinary/extra (no recalcula balances ni end_date).
-- Usa total_hours del snapshot + perfil; manager/fixed: extra = max(0, total-40).
update public.weekly_snapshots ws
set
  ordinary_hours = case
    when extract(month from ws.week_start) = 8 then 0
    when coalesce(p.role, 'staff') = 'manager'
      or coalesce(p.is_fixed_salary, false) then 0
    else least(
      coalesce(ws.total_hours, 0),
      coalesce(ws.contracted_hours_snapshot, 0)
    )
  end,
  extra_hours = case
    when extract(month from ws.week_start) = 8 then coalesce(ws.total_hours, 0)
    when coalesce(p.role, 'staff') = 'manager'
      or coalesce(p.is_fixed_salary, false) then
      greatest(0, coalesce(ws.total_hours, 0) - 40)
    else greatest(
      0,
      coalesce(ws.total_hours, 0) - coalesce(ws.contracted_hours_snapshot, 0)
    )
  end

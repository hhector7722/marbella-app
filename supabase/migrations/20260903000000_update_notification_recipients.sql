-- Migration: 20260903000000_update_notification_recipients.sql
-- Descripción: Sustituye 'pere' por 'fernando' como destinatario de las
-- notificaciones in-app (campana) de reservas y pedido cliente, alineándolo
-- con ALLOWED_FIRST_NAMES del webhook /api/webhooks/reservations-push.
...
BEGIN
  ...
  -- In-app: mismos destinatarios que reservas (campana NotificationsBell)
  INSERT INTO public.user_notifications (
    user_id, type, title, body, action_url, entity_type, entity_id
  )
  SELECT
  ...
  -- Push: mismo webhook / destinatarios que reservas
  SELECT value INTO v_webhook_url FROM public.app_settings
  WHERE key = 'push_webhook_url';
  ...
  IF v_webhook_url IS NOT NULL THEN
    v_record_json := jsonb_build_object(
      'type', 'INSERT',
      'table', 'events',
      'kind', 'client_order_submitted',
      'record', jsonb_build_object(
        'id', v_event_id,
        'customer_name', coalesce(v_event_name, 'Cliente'),
        'event_date', v_event_date::text,
        'event_time', to_char(v_event_time, 'HH24:MI'),
        'guest_count', v_guest_count,
        'kind_line', v_kind_line,

...
function runSql(sql) {
  ...
  try {
    const res = execSync(`npx supabase db query "${escapedSql}" --linked`, { encoding: 'utf8' });
    return res;
  } catch (err) {
...
async function run() {
  ...
  const setupSql = `
    ...
    BEGIN
      ...
      -- 1. purchase_invoices
      INSERT INTO public.purchase_invoices (supplier_id, invoice_number, invoice_date, status, source)
      VALUES (v_supplier_id, 'TEST-DELETE-001', '2026-08-12', 'pending_mapping', 'scanner')
  ...
  console.log(runSql(setupSql));

  // Now let's try to delete the invoice and see the exact error.
  console.log("=== 2. Attempting to delete the invoice ===");
  const deleteSql = `
    DO $$
    DECLARE
      v_invoice_id UUID;
